    ...
    def __getitem__(self, item) -> Tuple[BatchEncoding, List[BatchEncoding], BatchEncoding]:
        ...
        if self.mode in ["unified", "embedding"]:
            ...
            if len(self.ds_embedding[item]['neg']) < self.args.train_group_size - 1:
                if len(self.ds_embedding[item]['neg']) == 0: # here modified by jinkin, randome sample from other
                    num = self.args.train_group_size - 1
                    negs = [self.ds_embedding[item-1]['pos'][-1] for _ in range(num)]
                else:
